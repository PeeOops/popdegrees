...
const MovieDetails = () => {
    ...
    return( 
        <div className="flex flex-row">
            {/* Movie Poster */}
            <div className="bg-white h-screen w-1/3 p-8 flex items-start flex-col relative z-10">
                <i className="fa-solid fa-arrow-left mb-8 cursor-pointer" onClick={() => navigate(-1)} > Back</i>
                {loading ? <div className="loader border-4 border-t-4 border-gray-200 border-t-blue-500 rounded-full w-16 h-16 animate-spin m-auto"></div> : <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.name} className="rounded-lg 2xl:w-96 lg:w-80 m-auto"/>}
            </div>
            <div style={{backgroundImage : `url(https://media.themoviedb.org/t/p/w1920_and_h800_multi_faces${movie.backdrop_path})`}} className="bg-cover bg-center bg-red-950 h-screen w-2/3 p-8 text-white montserrat overflow-x-hidden">
